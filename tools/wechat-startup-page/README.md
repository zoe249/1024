...
启动页只有背景和 Logo 使用图片。“初始化中”和健康游戏忠告由微信离屏 2D Canvas
使用系统字体实时绘制，再交给 WebGL 展示，不再携带任何文字图片文件。

背景和 Logo 使用发布专用轻量资源，避免两张高清原图直接进入微信主包。安装脚本会
...
4. 清理旧版大图、旧文字图片和启动页预览文件
5. 统计主包体积，超过 4MB 时输出警告
6. 保留 Cocos 生成的 `game.js`、分包配置和游戏代码不变
